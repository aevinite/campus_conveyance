-- 0059_agency_onboard_count.sql (idempotent)
-- Manage Students paginates an agency's CONFIRMED bookings, but hidden (removed)
-- students are filtered out per page in JS. The "Page X of Y" pager used
-- agency_bookings_count(CONFIRMED), which still counts the hidden students'
-- bookings — so Y could be too high and a trailing page render empty. Fetching
-- with agency_bookings(CONFIRMED) had the same problem: hidden rows consumed
-- limit/offset slots, so a page could show fewer than PAGE_SIZE onboard cards.
--
-- These two RPCs paginate and count CONFIRMED bookings while excluding students
-- in agency_hidden_students, so the list, the page fill, and the pager total all
-- agree with what Manage Students actually shows.
