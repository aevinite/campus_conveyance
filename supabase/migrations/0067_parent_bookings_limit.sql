-- 0067_parent_bookings_limit.sql (idempotent)
-- parent_children_bookings() returned EVERY booking of every linked child with
-- no LIMIT — unbounded over years of daily trips (realistically small, but
-- bound it defensively). Cap at the 200 most recent across the parent's
-- children (newest first) — plenty for the dashboard, and the query stays
-- bounded regardless of history. Body is 0047's, only `limit 200` added.
