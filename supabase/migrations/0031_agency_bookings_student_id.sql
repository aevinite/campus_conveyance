-- 0031_agency_bookings_student_id.sql (idempotent)
-- Expose student_id on agency_bookings so the agency panel can (a) build the
-- "onboard students" roster from CONFIRMED bookings and (b) drive the per-student
-- hide/delete action straight from a booking row. Everything else is unchanged
-- from 0029 (drop + recreate is required because the return type changes).
drop function if exists public.agency_bookings(uuid);
create or replace function public.agency_bookings(p_agency_id uuid)
returns table (
  booking_id uuid, student_id uuid, status text, created_at timestamptz, is_paid boolean, paid_at timestamptz,
  ...
  select b.id, b.student_id, b.status::text, b.created_at, b.is_paid, b.paid_at,
